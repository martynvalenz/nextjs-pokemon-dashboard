...
export async function generateStaticParams() {
  const limit = 151;
  const data:PokemonsResponse = await fetch(`https://pokeapi.co/api/v2/pokemon?limit=${limit}`)
  .then(res => res.json());
...
const getPokemon = async(name:string):Promise<Pokemon> => {
  try {
    const pokemon = await fetch(`https://pokeapi.co/api/v2/pokemon/${name}`,{
      // cache:'force-cache', //TODO: change this later
      next:{
            ...
            <div className="flex flex-wrap">
              {
                pokemon.moves.map(move => (
                  ...
                  <span className="capitalize bg-blue-100 text-gray-800 text-md font-medium m-1 px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300" key={move.move.name}>{move.move.name}</span>
                ))
              }
            </div>
          </div>
